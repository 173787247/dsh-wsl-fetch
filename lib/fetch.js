...
export const DEFAULT_USER_AGENT = "deepseek-harness-wsl-fetch/0.1.0 (+https://github.com/173787247/dsh-wsl-fetch)";
...
function translateNetwork(error, signal) {
  ...
  return Object.assign(new Error(`web fetch failed: ${msg}`), { code: "WEB_PROVIDER_ERROR", cause: error });
}
      ...
      for (;;) {
        ...
        try {
          if (isRedirectStatus(response.status)) {
            ...
            if (!isSameOrigin(next, current)) {
              await response.body?.cancel();
              throw Object.assign(new Error(`cross-origin redirect to ${next.origin} is not followed automatically; retry against that URL directly`), { code: "WEB_REDIRECT_BLOCKED" });
            }
            await response.body?.cancel();
            current = next;
            redirects += 1;
